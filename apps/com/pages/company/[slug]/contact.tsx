...
import { CompanyContactPage, prefetch } from '@homeproved/com/feature-company';
...
export const Contact: PageWithAuthorization = () => {
  const router = useRouter();
  const { slug } = router.query;

  return (
    <PageShell tabsMenu footerMargin={0}>
      <CompanyContactPage slug={slug as string} />
    </PageShell>
  );
};
...
export async function getServerSideProps({ locale, params: { slug } }) {
  ...
  return {
    props: {
      dehydratedState: await getDehydratedState(prefetch.contact(locale, slug)),
